...
class ExcelTab(BaseTab):
    ...
    def _read_event_names_from_excel(self) -> None:
        ...
        def task() -> None:
            ...
            if has_existing:
                self.log(f"✅ 行事名を上書きしました（合計: {total}件）", "success")
                self.update_status(f"✅ 行事名を{total}件上書きしました")
                dialog_title = "✅ 上書き完了"
                preamble = "既存の設定を上書きしました。"
            else:
                self.log(
        ...
        self.run_in_thread(
            task,
            button=self.read_event_button,
            running_status="読み込み中...",
            error_title="読み込みエラー",
        )
