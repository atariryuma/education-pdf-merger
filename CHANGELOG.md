...
1. **目次のページ番号と実コンテンツのずれを修正** ([document_collector.py](document_collector.py), [pdf_merge_orchestrator.py](pdf_merge_orchestrator.py))
   - **問題**: 目次のリンク先ページが実際のコンテンツとずれていた
   - **原因1**: `sorted()` のUnicode順では「表紙」ファイル名が数字始まりフォルダの後に来るため、`content_pdfs`の先頭に表紙が来ず、`split_pdf` が最初の区切りページを表紙として誤って切り出していた
   - **原因2**: 表紙ファイルが存在しない場合でも`split_pdf`が最初のコンテンツを表紙として切り出し、目次ページ番号がずれていた
   - **修正**:
     - 表紙ファイルを抽出して`content_pdfs`の先頭に必ず配置
     - 表紙が無い場合は`split_pdf`をスキップしてコンテンツ全体を使用
     - 表紙無し時のコンテンツ開始ページを正しく計算（目次直後の2ページ目）
