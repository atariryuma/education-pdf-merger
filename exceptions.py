...
class GoogleSheetsError(PDFMergeError):
    """Google Sheets API操作エラー"""
    def __init__(
        self,
        message: str,
        *,
        ...
        Args:
            message: エラーメッセージ
            operation: 実行していた操作（例: "データ読み取り", "シート検索"）
            original_error: 元の例外
        """
        if operation:
            full_message = f"Google Sheets {operation}エラー: {message}"
        else:
            full_message = f"Google Sheetsエラー: {message}"

        super().__init__(full_message, original_error=original_error, operation=operation)


class GoogleAuthError(PDFMergeError):
    """Google OAuth認証エラー"""
    def __init__(
        self,
        message: str,
        *,
        ...
        Args:
            message: エラーメッセージ
            auth_stage: 認証のどの段階でエラーが発生したか（例: "初回認証", "トークン更新"）
            original_error: 元の例外
        """
        if auth_stage:
            full_message = f"Google認証エラー [{auth_stage}]: {message}"
        else:
            full_message = f"Google認証エラー: {message}"

        super().__init__(full_message, original_error=original_error, auth_stage=auth_stage)
